...
def main():
    ...
    # CHANGE 1: Read arguments defining the curriculum
    # Define curriculum
    epoch_curr = np.asarray(args.epochs)
    print('epochs: ', epoch_curr)
    sigma_curr = np.asarray(args.sigmas)
    print('sigmas: ', sigma_curr)
    curriculum_dict = {}
    for i, ep in enumerate(np.cumsum(epoch_curr)):
        curriculum_dict[ep] = sigma_curr[i]
    ...
    # CHANGE 2: Total number of epochs = sum of epochs of segments of the curriculum
    for epoch in range(0, args.epochs.sum()):

        # CHANGE 3: 
        # Pick kernel, sigma value for current epoch as per curriculum
        idx = next(x for x, xval in enumerate(np.cumsum(epoch_curr)) if xval > epoch)
        ...
        train_loss, epoch_time = train(train_loader, model, criterion, optimizer, epoch, ([6*e+1 for e in sigma_curr[:idx+1]]), 
                                       sigma_curr[:idx+1], np.cumsum(epoch_curr)[:idx+1]/sum(np.cumsum(epoch_curr)[:idx+1]))
...
def train(train_loader, model, criterion, optimizer, epoch, kernel_arr, sigma_arr, weightage):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # blur input images
        # input = blur(input, kernel_arr, sigma_arr, weightage)
        idx = np.random.choice(np.arange(len(kernel_arr)), size=1, p=weightage)[0]
        # apply blur
        input = transforms.functional.gaussian_blur(input, int(kernel_arr[idx]), sigma_arr[idx].astype('float')) if sigma_arr[idx].astype('float') > 0 else sample
