...
def main():
    ...
    # these are the additional train transforms to be performed before test transforms
    train_transform = transforms.Compose([
        transforms.RandomCrop(32, padding=4),
        transforms.RandomHorizontalFlip()
    ])
    ...
    train_loader_0 = torch.utils.data.DataLoader(
        AugMixDataset(train_data_0, preprocess, args.no_jsd),
    ...
    if args.model == 'densenet':
        ...
        net = WideResNet(args.layers, num_classes,
                         args.widen_factor, args.droprate)
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        ...
        lr_lambda=lambda step: get_lr(  # pylint: disable=g-long-lambda
            step,
            args.epochs * len(train_loader_0),
    ...
    for epoch in range(start_epoch, epoch_cumul_curr[-1]):
        begin_time = time.time()

        # Pick kernel, sigma value for current epoch as per curriculum
        idx = next(x for x, xval in enumerate(
            epoch_cumul_curr) if xval > epoch)
        ...
        train_data = AugMixDataset(train_data_0, preprocess_blur, args.no_jsd)
