...
"""
Save a checkpoint after every segment of the curriculum
"""
...
def save_checkpoint(state, is_best, sigma):
    filename = f'checkpoint_sigma{sigma}.pth.tar'
    ...
    if is_best:
        shutil.copyfile(filename, 'runs/%s/' % (args.expname) + f'model_best_sigma{sigma}.pth.tar')
