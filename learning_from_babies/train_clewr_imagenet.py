...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Define curriculum
    # epoch_curr = [3, 3, 6, 24, 54]

    sigma_curr = [16, 8, 4, 2, 1, 0]
    epoch_curr = [1, 1, 2, 5, 9, 72]
    kernel_curr = [4*e+1 for e in sigma_curr]

    epoch_cumul_curr = np.cumsum(np.asarray(epoch_curr))
    print(epoch_cumul_curr)
    args.epochs = epoch_cumul_curr[-1]

    for epoch in range(args.start_epoch, epoch_cumul_curr[-1]):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, kernel_arr, sigma_arr, weightage)
        ...
        out_dir = os.path.join('runs', args.expname)
...
def train(train_loader, model, criterion, optimizer, epoch, args, kernel_arr, sigma_arr, weightage):
