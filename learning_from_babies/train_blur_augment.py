...
parser.add_argument('--sigma_max', default=0, type=int,
                    help = 'max sigma for gaussian blurring')
...
def main():
    ...
    for epoch in range(0, args.epochs):
        ...
        # train for one epoch
        train_loss, epoch_time = train(train_loader, model, criterion, optimizer, epoch, args.sigma_max)
...
class BlurAug(object):
    ...
    def __call__(self, sample):
        ...
        # decide whether to blur or not: 80% chance of not blurring
        b = np.random.choice([0, 1], size=1, p=[0.8, 0.2])
        # print('b = ', b)
        
        if b == 1:
            # pick a blur level
            sigma = np.random.randint(self.sigma_max, size=1)[0] + 1
            # sigma = np.random.choice(self.sigma_arr, size=1, p=self.prob_arr)[0]
            # print('simga, kernel ', sigma, kernel)

            kernel = 6*sigma + 1

            # apply blur
            if sigma > 0:
                sample = transforms.functional.gaussian_blur(sample, kernel.item(), sigma.item()) 

        return sample


def train(train_loader, model, criterion, optimizer, epoch, sigma_max):
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
